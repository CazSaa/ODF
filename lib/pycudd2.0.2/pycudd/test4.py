...
def test_formula_equivalence():
    ...
    var_names1 = ["LP", "LJ", "DF", "HS", "IU", "DSL", "LGJ", "FBO", "PL", "DD",
                  "EDLU"]
    var_names2 = ["DD", "DF", "DSL", "EDLU", "FBO", "HS", "IU", "LGJ", "LJ",
                  "LP", "PL"]
...
def replace_specific_nodes_rebuild(m, add, config_reflection_nodes, idx_to_name,
                                   vars):
    """
    ...
    config_refl_set = config_reflection_nodes
    ...
    def rebuild_node(node, depth=0):
        ...
        if node.IsConstant():
            print(
                f"{indent}[Depth {depth}] Node {hex(node.GetNodeId())} is a terminal node with value {node.V()}")
        ...
        if node in processed_nodes:
            print(
                f"{indent}[Depth {depth}] Node {hex(node.GetNodeId())} already processed")
            return processed_nodes[node]

        node_index = node.NodeReadIndex()
        node_name = idx_to_name.get(node_index, "unknown")
        print(
            f"{indent}[Depth {depth}] Processing node {node_name}({hex(node.GetNodeId())})")
        ...
        if (node, then_child) in config_refl_set:
            print(
                f"{indent}[Depth {depth}] THEN edge will be replaced with custom constant")
            new_then = new_const_node()

            parent_index = node.NodeReadIndex()
            parent_name = idx_to_name.get(parent_index, "unknown")
            child_index = then_child.NodeReadIndex()
            child_name = idx_to_name.get(child_index,
                                         "unknown") if not then_child.IsConstant() else "Terminal"
            print(
                f"{indent}[Depth {depth}] Replacing THEN edge from {parent_name}({hex(node.GetNodeId())}) "
                f"to {child_name}({hex(then_child.GetNodeId())}) with {new_then.V()}({hex(new_then.GetNodeId())})")
        else:
            print(f"{indent}[Depth {depth}] Recursing on THEN child")
            new_then = rebuild_node(then_child, depth + 1)
            print(f"{indent}[Depth {depth}] Returned from THEN recursion")

        # Process ELSE branch
        if (node, else_child) in config_refl_set:
            print(
                f"{indent}[Depth {depth}] ELSE edge will be replaced with custom constant")
            new_else = new_const_node()

            parent_index = node.NodeReadIndex()
            parent_name = idx_to_name.get(parent_index, "unknown")
            child_index = else_child.NodeReadIndex()
            child_name = idx_to_name.get(child_index,
                                         "unknown") if not else_child.IsConstant() else "Terminal"
            print(
                f"{indent}[Depth {depth}] Replacing ELSE edge from {parent_name}({hex(node.GetNodeId())}) "
                f"to {child_name}({hex(else_child.GetNodeId())}) with {new_else.V()}({hex(new_else.GetNodeId())})")
        else:
            print(f"{indent}[Depth {depth}] Recursing on ELSE child")
            new_else = rebuild_node(else_child, depth + 1)
            print(f"{indent}[Depth {depth}] Returned from ELSE recursion")

        # Create a new node with the possibly modified children
        var = m.addIthVar(node_index)
        new_node = var.addIte(new_then, new_else)
        print(
            f"{indent}[Depth {depth}] Created new node {hex(new_node.GetNodeId())} by combining "
            f"new_then={hex(new_then.GetNodeId())} and new_else={hex(new_else.GetNodeId())}")
        # m.KillNode(then_child)
        # m.KillNode(else_child)
...
def test_mtbdd(m: Optional[pycudd.DdManager] = None):
    ...
    for node in dfs(formula):
        ...
        print(
            f"node: {hex(int(node))} ;; index: {index} ;; name: {name} ;; value: {value} ;; is_complemented: {is_complemented}")
        ...
        for child in [node.T(), node.E()]:
            ...
            if child.IsConstant():
                ...
                print(
                    f"  Found reflection terminal: {child_value} ;; is_complemented: {child_is_complemented}")
                continue

            child_index = child.NodeReadIndex()
            child_name = idx_to_name[child_index]
            if name in object_properties and (
                    child_name in ft_nodes or child_name in at_nodes):
                print(
                    f"  Found reflection node: {child_name} ;; node: {hex(child.GetNodeId())}")
...
def test_mtbdd_smaller(m: Optional[pycudd.DdManager] = None):
    ...
    for node in dfs(add):
        ...
        is_complemented = node.IsComplement()
        print(
            f"node: {hex(node.GetNodeId())} ;; index: {index} ;; name: {name} ;; value: {value} ;; is_complemented: {is_complemented}")
        if node.IsConstant(): continue
    ...
    replace_specific_nodes_rebuild(m, add, config_reflection_nodes, idx_to_name,
                                   vars)
