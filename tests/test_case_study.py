...
def test_case_study(do_case_study_layer2, case_study_configuration_str,
    ...
    add_cmd("PipelineRelativeIncrease", orig_pipeline_relative_inc, precision=2) # Example: 44.90% -> 44.90
    ...
    add_cmd("WaterhammerPipelineMaxRiskPercentage", orig_waterhammer_contrib_max_risk_percent)
    ...
    adapt_pipeline_risk_percentage_increase = relative_increase(adapt_pipeline_min_total_risk, adapt_pipeline_max_total_risk) * 100 # from L177-179
    add_cmd("PipelineRiskPercentageIncreaseAdapt", adapt_pipeline_risk_percentage_increase, precision=2)

    if adapt_pipeline_max_total_risk != 0:
        adapt_wa_max_risk_fraction = (wao_max_risk / adapt_pipeline_max_total_risk) * 100 # from L223
    else:
        adapt_wa_max_risk_fraction = float('inf')
    add_cmd("WaterhammerAttackMaxRiskFractionAdapt", adapt_wa_max_risk_fraction, precision=2)
    
    if adapt_pipeline_min_total_risk != 0:
        adapt_wa_min_risk_fraction = (wao_min_risk / adapt_pipeline_min_total_risk) * 100 # from L227
    else:
        adapt_wa_min_risk_fraction = float('inf')
    add_cmd("WaterhammerAttackMinRiskFractionAdapt", adapt_wa_min_risk_fraction, precision=2)

    # These variables (environment_risk_increase, etc.) were calculated using alternative_case_study_models
    add_cmd("EnvironmentFactorIncreaseAdapt", environment_risk_increase, precision=2) # L259
    add_cmd("PipelineFactorIncreaseAdapt", pipeline_risk_increase, precision=2) # L264
    add_cmd("SCADARiskFactorIncreaseAdapt", scada_risk_increase, precision=2) # L269
    add_cmd("RTURiskFactorIncreaseAdapt", rtu_risk_increase, precision=2) # L274
    
    add_cmd("ACPORemoteCCRiskIncreaseAdapt", acpo_remote_cc_risk_increase, precision=2) # L294
    add_cmd("APORemoteCCRiskIncreaseAdapt", apo_remote_cc_risk_increase, precision=2) # L299
    
    add_cmd("ACPOAllowFirmwareRiskIncreaseAdapt", acpo_allow_fw_risk_increase, precision=0) # L305
    add_cmd("APOAllowFirmwareRiskIncreaseAdapt", apo_allow_fw_risk_increase, precision=0) # L310

    # Determine output path
    current_script_dir = os.path.dirname(os.path.abspath(__file__))
    # Path should be ../../report/chapters/case-study/ relative to tests/ directory
    output_dir = os.path.join(current_script_dir, "..", "..", "report", "chapters", "case-study")
    
    # Ensure the output directory exists
    os.makedirs(output_dir, exist_ok=True)
    
    output_file_path = os.path.join(output_dir, "case_study_generated_values.tex")
...
def run_cli_for_latex(odf_file_path_relative_to_project_root):
    ...
    command = CLI_COMMAND_TEMPLATE.format(odf_file_path_relative_to_project_root)
    try:
        # project_root is one level up from the 'tests' directory where this script resides
        project_root = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..")
        print(f"Running CLI: {command} in {project_root} for ODF file: {odf_file_path_relative_to_project_root}")
        ...
        print(f"Error running CLI for {odf_file_path_relative_to_project_root}: {e}")
        print(f"Stdout: {e.stdout}")
        print(f"Stderr: {e.stderr}")
        return None
    except FileNotFoundError:
        # This error means 'python' itself (or the first part of the command) wasn't found.
        print(f"Error: The command '{command.split()[0]}' was not found. Ensure Python is in PATH and the virtual environment (if any) is active.")
        return None
    except Exception as e:
        print(f"An unexpected error occurred while running CLI for {odf_file_path_relative_to_project_root}: {e}")
        return None
...
def parse_cli_output_for_latex(raw_output):
    ...
    for match in pattern.finditer(raw_output):
        full_block_content = match.group(1).strip() # group(1) is the entire block
...
def generate_cli_output_listing_files(all_formula_outputs_data):
    ...
    output_subdir_abs_path = os.path.abspath(os.path.join(script_dir, CLI_OUTPUT_FILES_SUBDIR))

    try:
        os.makedirs(output_subdir_abs_path, exist_ok=True)
        print(f"Ensured CLI output subdirectory exists: {output_subdir_abs_path}")
    except OSError as e:
        print(f"Error creating CLI output subdirectory '{output_subdir_abs_path}': {e}")
    ...
    for item_data in all_formula_outputs_data:
        ...
        if not blocks:
            print(f"Note: No formula blocks found for ODF file associated with prefix '{item_data['prefix']}'. No .tex files generated for this set.")
            continue
        ...
        for number_str, raw_block_str in blocks.items():
            try:
                ...
                print(f"Error: Formula number '{number_str}' is not a valid integer. Skipping file generation for this formula under {item_data['prefix']}.")
                continue
            except IOError as e:
                print(f"Error writing CLI output file '{file_abs_path}': {e}")
            except Exception as e:
                print(f"An unexpected error occurred while generating file for formula {number_str} ('{item_data['prefix']}'): {e}")
...
def generate_cli_output_latex():
    ...
    for odf_file_item in ODF_FILES_INFO: # ODF_FILES_INFO contains 'path' and 'prefix'
        print(f"Processing ODF file for LaTeX CLI output: {odf_file_item['path']}")
        ...
        if raw_cli_output:
            ...
            if parsed_blocks:
                ...
                print(f"Successfully parsed {len(parsed_blocks)} formula blocks from {odf_file_item['path']}.")
            else:
                print(f"Warning: No formula blocks parsed from the output of {odf_file_item['path']}.")
        else:
            print(f"Failed to get CLI output for {odf_file_item['path']}. No .tex files will be generated from this ODF.")
            
        all_parsed_data_for_latex.append({
            "prefix": odf_file_item["prefix"], # Used to determine if it's "adapt" or not for filename
            "path": odf_file_item["path"],     # For comments in the generated files
    ...
    if any(item["blocks"] for item in all_parsed_data_for_latex):
        ...
        print("No CLI output was successfully parsed from any ODF file. No individual .tex listing files generated.")
